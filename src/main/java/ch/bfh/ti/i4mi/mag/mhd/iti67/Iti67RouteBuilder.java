...
class Iti67RouteBuilder extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:mdh-documentreference-adapter-GET")
                .bean(IdRequestConverter.class)
                .to(metadataQueryEndpoint)
                .process(translateToFhir(iti67ResponseConverter, QueryResponse.class))
        ;

        from("direct:mdh-documentreference-adapter-PUT")
                .process(exchange -> {
        ...
        from("direct:mdh-documentreference-adapter-DELETE")
                .process(exchange -> {
